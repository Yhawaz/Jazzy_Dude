...
*
*******************************************************************************/
void sax_clk_Start(void) 
{
    /* Set the bit to enable the clock. */
    sax_clk_CLKEN |= sax_clk_CLKEN_MASK;
	sax_clk_CLKSTBY |= sax_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: sax_clk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    sax_clk_CLKEN &= (uint8)(~sax_clk_CLKEN_MASK);
	sax_clk_CLKSTBY &= (uint8)(~sax_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: sax_clk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_StopBlock(void) 
{
    if ((sax_clk_CLKEN & sax_clk_CLKEN_MASK) != 0u)
    {
...
#if defined(sax_clk__CFG3)
        CLK_DIST_AMASK = sax_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = sax_clk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(sax_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(sax_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: sax_clk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        sax_clk_CLKSTBY &= (uint8)(~sax_clk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: sax_clk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = sax_clk_GetSourceRegister();
    uint16 oldDivider = sax_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = sax_clk_CLKEN & sax_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((sax_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: sax_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 sax_clk_GetDividerRegister(void) 
{
    return CY_GET_REG16(sax_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: sax_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_SetModeRegister(uint8 modeBitMask) 
{
    sax_clk_MOD_SRC |= modeBitMask & (uint8)sax_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: sax_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_ClearModeRegister(uint8 modeBitMask) 
{
    sax_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(sax_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: sax_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 sax_clk_GetModeRegister(void) 
{
    return sax_clk_MOD_SRC & (uint8)(sax_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: sax_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = sax_clk_GetDividerRegister();
    uint8 oldSrc = sax_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        sax_clk_MOD_SRC |= CYCLK_SSS;
        sax_clk_MOD_SRC =
            (sax_clk_MOD_SRC & (uint8)(~sax_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: sax_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 sax_clk_GetSourceRegister(void) 
{
    return sax_clk_MOD_SRC & sax_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: sax_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void sax_clk_SetPhaseRegister(uint8 clkPhase) 
{
    sax_clk_PHASE = clkPhase & sax_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: sax_clk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 sax_clk_GetPhaseRegister(void) 
{
    return sax_clk_PHASE & sax_clk_PHASE_MASK;
}
