...
void piano_clk_Start(void) 
{
    ...
    piano_clk_CLKEN |= piano_clk_CLKEN_MASK;
	piano_clk_CLKSTBY |= piano_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: piano_clk_Stop
********************************************************************************
*
* Summary:
...
void piano_clk_Stop(void) 
{
    ...
    piano_clk_CLKEN &= (uint8)(~piano_clk_CLKEN_MASK);
	piano_clk_CLKSTBY &= (uint8)(~piano_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: piano_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((piano_clk_CLKEN & piano_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = piano_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = piano_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* piano_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(piano_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(piano_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: piano_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: piano_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = piano_clk_GetSourceRegister();
    uint16 oldDivider = piano_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = piano_clk_CLKEN & piano_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((piano_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: piano_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(piano_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: piano_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    piano_clk_MOD_SRC |= modeBitMask & (uint8)piano_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: piano_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    piano_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(piano_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: piano_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return piano_clk_MOD_SRC & (uint8)(piano_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: piano_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = piano_clk_GetDividerRegister();
    uint8 oldSrc = piano_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        piano_clk_MOD_SRC |= CYCLK_SSS;
        piano_clk_MOD_SRC =
            (piano_clk_MOD_SRC & (uint8)(~piano_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: piano_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return piano_clk_MOD_SRC & piano_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: piano_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    piano_clk_PHASE = clkPhase & piano_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: piano_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return piano_clk_PHASE & piano_clk_PHASE_MASK;
}
