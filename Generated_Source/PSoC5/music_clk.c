...
void music_clk_Start(void) 
{
    ...
    music_clk_CLKEN |= music_clk_CLKEN_MASK;
	music_clk_CLKSTBY |= music_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: music_clk_Stop
********************************************************************************
*
* Summary:
...
void music_clk_Stop(void) 
{
    ...
    music_clk_CLKEN &= (uint8)(~music_clk_CLKEN_MASK);
	music_clk_CLKSTBY &= (uint8)(~music_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: music_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((music_clk_CLKEN & music_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = music_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = music_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* music_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(music_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(music_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: music_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: music_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = music_clk_GetSourceRegister();
    uint16 oldDivider = music_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = music_clk_CLKEN & music_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((music_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: music_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(music_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: music_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    music_clk_MOD_SRC |= modeBitMask & (uint8)music_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: music_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    music_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(music_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: music_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return music_clk_MOD_SRC & (uint8)(music_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: music_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = music_clk_GetDividerRegister();
    uint8 oldSrc = music_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        music_clk_MOD_SRC |= CYCLK_SSS;
        music_clk_MOD_SRC =
            (music_clk_MOD_SRC & (uint8)(~music_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: music_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return music_clk_MOD_SRC & music_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: music_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    music_clk_PHASE = clkPhase & music_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: music_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return music_clk_PHASE & music_clk_PHASE_MASK;
}
